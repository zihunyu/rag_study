...
def test_cli_writes_content_free_manifest_report(tmp_path: Path):
    ...
    (tmp_path / "meta.yaml").write_text(
        yaml.safe_dump({"samples": [{"file": "a.bin", "expected_locators": []}]}), encoding="utf-8"
    )
    ...
    manifest.write_text(
        yaml.safe_dump(
            {
                "collection_plan": [
                    {"format": "x", "metadata_path": "meta.yaml", "sample_directory": "data"}
                ]
            }
        ),
        encoding="utf-8",
    ...
    script = Path(__file__).resolve().parents[2] / "scripts/scan_fixture_render_coverage.py"
