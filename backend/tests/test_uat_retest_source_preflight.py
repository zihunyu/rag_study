...
def test_generated_preflight_cli(tmp_path: Path):
    review = tmp_path / "r"
    review.write_text(
        json.dumps({"type": "candidate_review", "audit_verdict": "不通过", "candidate_id": "a"})
        + "\n"
    )
    scan = tmp_path / "s"
    scan.write_text(
        json.dumps({"records": [{"fixture_ref": "x", "representation_status": "AVAILABLE"}]})
    )
    cases = tmp_path / "c"
    cases.write_text(
    ...
    assert json.loads(out.read_text())["eligible_count"] == 1
