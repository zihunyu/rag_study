...
def test_upload_parse_chunk_embed_index_search_and_ask(tmp_path: Path) -> None:
    ...
    staged_search = client.post("/api/search", json={"query": "ThinkPad 21FA 保修期"})
    assert staged_search.status_code == 200
    assert staged_search.json()["hits"] == []
    review = client.post(
        f"/api/document-versions/{completed['document_version_id']}/review",
    ...
    published = client.post(
        f"/api/document-versions/{completed['document_version_id']}:publish",
        headers={"Idempotency-Key": "e2e-publish"},
    )
    assert published.status_code == 200

    search = client.post("/api/search", json={"query": "ThinkPad 21FA 保修期"})
    answer = client.post("/api/ask", json={"question": "ThinkPad 21FA 保修期多久？"})
