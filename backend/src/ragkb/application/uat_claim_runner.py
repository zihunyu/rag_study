"""Structured-claim acceptance with isolated runs and immutable evidence bindings."""
...
class UatClaimRunner:
    """Execute a bounded case set, preserving checkpoints within the selected run."""

    revision = "uat-claim-runner"
    ...
    def __init__(
        ...
        *,
        ...
        self.namespace = f"uat_claim:{run_id}"
        self.artifact_revision = run_id
    ...
    def _prepare_cases(self, cases: Sequence[Mapping[str, object]]) -> list[_PreparedCase]:
        if not cases or self.max_requests < len(cases):
            raise ProviderExecutionError("UAT_CLAIM_REQUEST_BUDGET_INVALID")
        prepared = [self._prepared_case(case) for case in cases]
        case_ids = [item["binding"]["test_case_id"] for item in prepared]
        if len(set(case_ids)) != len(case_ids):
            raise ProviderExecutionError("UAT_CLAIM_CASE_ID_DUPLICATE")
        return prepared

    def plan(self, cases: Sequence[Mapping[str, object]]) -> dict[str, object]:
        """Validate the complete case set without storing results or calling a provider."""
        prepared = self._prepare_cases(cases)
        return {
            "revision": self.revision,
            "run_id": self.artifact_revision,
            "case_count": len(prepared),
            "max_requests": self.max_requests,
            "input_snapshot_sha256": canonical_sha256([item["binding"] for item in prepared]),
            "automatic_retries": 0,
            "executed": False,
    ...
    def _case_id(value: object) -> str:
        if not isinstance(value, str) or not _SAFE_CASE_ID.fullmatch(value):
            raise UatRemediationError("UAT_CASE_ID_INVALID")
        return value
    ...
    def _raw_envelope(value: Mapping[str, object]) -> dict[str, object]:
        ...
        if entity is not None and not isinstance(entity, str):
            raise UatRemediationError("UAT_ENTITY_ID_INVALID")
        if field is not None and not isinstance(field, str):
            raise UatRemediationError("UAT_FIELD_KEY_INVALID")
        locator = value["locator"]
        if not isinstance(locator, Mapping):
            raise UatRemediationError("UAT_LOCATOR_INVALID")
    ...
    def _prepared_case(self, value: Mapping[str, object]) -> _PreparedCase:
        ...
        if set(value) != required:
            raise UatRemediationError("UAT_CASE_SCHEMA_INVALID")
        ...
        if (
            ...
            raise UatRemediationError("UAT_CASE_SCHEMA_INVALID")
        envelopes = [
            self._raw_envelope(item) if isinstance(item, Mapping) else _raise_evidence_schema()
        ...
        for item in envelopes:
            ...
            if not isinstance(integrity, Mapping):
                raise UatRemediationError("UAT_SOURCE_INTEGRITY_INVALID")
    ...
    def _ensure_coverage(
        ...
        for case_id in case_ids:
            ...
            if checkpoint is None or checkpoint.get("state") != "COMPLETED":
                raise ProviderExecutionError("UAT_CLAIM_COVERAGE_INCOMPLETE")
            ...
            if not isinstance(audit_ref, str) or not isinstance(audit_sha256, str):
                raise ProviderExecutionError("UAT_CLAIM_COVERAGE_AUDIT_REF_MISSING")
        ...
        try:
            ...
            raise ProviderExecutionError(
                "UAT_CLAIM_COVERAGE_INVALID", outcome_unknown=False
            ) from error
    ...
    def run(self, cases: Sequence[Mapping[str, object]]) -> dict[str, object]:
        if self.transport.real_network and not self.external_call_approved:
            raise ExecutionApprovalRequired("UAT_CLAIM_EXECUTION_APPROVAL_REQUIRED")
        prepared = self._prepare_cases(cases)
        bindings = [dict(item["binding"]) for item in prepared]
        manifest_parameters = {
            "revision": self.revision,
        ...
        if manifest is None:
            ...
            raise ProviderExecutionError("UAT_CLAIM_SNAPSHOT_OR_PARAMETERS_MISMATCH")
        completed = 0
        for item in prepared:
            ...
            if checkpoint is not None:
                if any(checkpoint.get(key) != value for key, value in binding.items()):
                    raise ProviderExecutionError("UAT_CLAIM_CHECKPOINT_BINDING_MISMATCH")
                if checkpoint.get("state") == "COMPLETED":
                    completed += 1
                    continue
                raise ProviderExecutionError(
                    str(checkpoint.get("error_code", "UAT_CLAIM_PREVIOUS_ATTEMPT_BLOCKS_RESUME"))
                )
            if int(str(manifest["request_count"])) >= self.max_requests:
                raise ProviderExecutionError("UAT_CLAIM_REQUEST_BUDGET_EXCEEDED")
            checkpoint = {
                "state": "UNKNOWN_OUTCOME",
                **binding,
                "idempotency_key": f"uat-claim:{self.artifact_revision}:{case_id}",
                "automatic_retries": 0,
            }
            ...
            try:
                ...
                if not isinstance(response, Mapping):
                    raise ProviderExecutionError(
                        "UAT_CLAIM_RESPONSE_INVALID", outcome_unknown=False
                    )
                ...
                code = (
                    error.code if isinstance(error, ProviderExecutionError) else "UAT_CLAIM_FAILURE"
                )
                unknown = (
    ...
    *,
    ...
    for case in cases:
        ...
        if not isinstance(classification, str) or not classification:
            raise ExecutionApprovalRequired("UAT_CLAIM_CLASSIFICATION_INVALID")
...
def _raise_evidence_schema() -> Any:
    raise UatRemediationError("UAT_EVIDENCE_SCHEMA_INVALID")
