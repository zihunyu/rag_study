...
def join_confirmation(
    ...
    by_key = {(c["review_index"], c["source_bundle_sha256"]): c for c in cases}
    out = []
    for row in rows:
        key = (row["review_index"], row["source_bundle_sha256_from_case"])
        case = by_key[key]
        out.append(
            {
                "test_case_id": case["test_case_id"],
                "source_bundle_sha256": case["source_bundle_sha256"],
