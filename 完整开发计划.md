...
| 明确技术约束 | Python 3.12、Vue 3、MySQL、Milvus、MinerU、统一 AI 适配层 |
...
2. 技术路线冲突：前半已明确 MySQL + Milvus + MinerU，后半仍推荐 PostgreSQL、pgvector、Elasticsearch 和 Docling。
...
1. 以原文件前半明确的 Python 3.12 + Vue 3 + MySQL + Milvus + MinerU 为约束。
...
4. MySQL 是元数据、权限、任务和发布控制面的事实源；项目本地文件系统是原件与规范化内容的事实源；Milvus 是可重建检索投影。
5. 生产目标基线采用 Milvus 2.6.x Dense + 原生 BM25 稀疏检索和中文分析器；RRF 由应用层 `rag_orchestration` 负责。G0 冻结 Harness 和验收清单，真实中文、ACL 与 watermark 验收是 G2 硬门禁。
...
| 灾备 | 控制面与内容事实源 RPO / 核心搜索 RTO | ≤ 15 分钟 / ≤ 4 小时；需有 Milvus 恢复或可用副本方案 |
...
- Milvus Dense + BM25 双路召回、相同权限过滤、RRF、去重和重排；
...
flowchart LR
    ...
    APP --> MQ[(Python 本地持久队列)]
    MQ --> WORKER[Python Workers]
    WORKER --> PARSER[MinerU / OCR / Office 转换 / ASR]
    WORKER --> EMB[Embedding 适配器]
    WORKER --> MILVUS[(Milvus Dense + BM25)]
...
4. Milvus 只保存可重建检索投影，不作为原文或权限的唯一副本。
5. 文档版本不可原地修改；新版本独立加工、验证、发布和回滚。
6. 文档解析、OCR、ASR、Embedding 和批量索引全部由 Worker 执行。
7. 权限条件进入 Dense 与 BM25 检索表达式；Milvus 返回 ID 后、读取正文或调用 Reranker 前，必须由 MySQL 做权威批量复核，输出前再复核一次。
...
- MySQL、Milvus、Redis 如保留，只允许本机原生安装、现有原生服务或 Python 可调用的本地/嵌入式适配器。R1 默认任务队列是 SQLite/文件日志支持的 Python 本地持久队列；RabbitMQ/Celery 仅为可选原生适配器。缺少真实服务时使用显式 Stub，但不得用 Stub 通过相应真实 Gate。
...
| 检索 | Milvus 2.6.x | Dense + BM25 稀疏检索、按语言分析器；应用层执行 RRF |
...
| 任务 | Python 本地持久队列（SQLite/文件日志） | R1 默认；MySQL 任务表仍是权威状态，队列不保存唯一状态；RabbitMQ/Celery 是可选原生适配器 |
...
| ADR | 建议决策 | 批准状态 |
| --- | --- | --- |
| ADR-001 | 采用模块化单体 API + 独立 Worker；R1 默认 Python 本地持久队列，RabbitMQ/Celery 为可选适配器 | G0 已重基线 |
| ADR-002 | MySQL 为控制面事实源，本地文件系统为内容事实源，Milvus 为检索投影 | G0 已批准方向；本地 Stub 不代表真实组件验收 |
...
| ADR-007 | 生产目标 Milvus 原生 BM25 first；G1 本地词法适配器仅供开发，G2 执行真实中文/ACL/watermark 硬验收 | G0 已重基线 |
...
│   │   ├── vector_indexing/             # Embedding、Milvus、索引代际与重建
...
│   │   └── infrastructure/              # MySQL、Milvus、本地文件、MQ、Redis、模型客户端
│   ├── workers/
│   ├── configs/
│   ├── migrations/
│   ├── tests/
...
领域层不得直接依赖 FastAPI、SQLAlchemy、Milvus SDK、Celery 或 MinerU SDK。
...
| `index_entries` | `generation_id, chunk_id, milvus_pk, checksum, state`；用于对账 |
...
- Milvus 预过滤包含租户、空间、活动代际、`SERVING` 投影、时态、密级、适用条件和有效 ACL；“是否为逻辑文档当前版本”由召回 ID 后的 MySQL 批量校验完成。
...
- 阶段 0 验证 `ARRAY_CONTAINS_ANY` 的表达式长度和性能；Milvus 2.6.x ARRAY 的 `max_capacity` 创建时固定在 1—4096，R1 必须将有效授权 Token 控制在 Schema 上限内，超限资源改用更粗授权域或独立 Collection。

### 9.4 ACL 编译语义

R1 使用可预编译权限模型，不要求 Milvus 在查询时解析复杂继承：
...
3. MySQL 中显式 deny 优先于 allow；权限编译器计算最终有效授权域，Milvus 只保存最终 `effective_scope_tokens`，不解析 deny。
...
读取 MySQL active generation、release manifest、permission revision 和 security watermark
  ...
  → Milvus BM25/Dense 仅返回 chunk ID、rank 和非敏感元数据
...
Milvus 无法关联 `documents.current_version_id`，因此当前版本校验不能伪装成一个 Milvus 过滤条件。非当前版本 ID 即使因普通内容投影延迟被召回，也不得读取正文、发送到外部 Reranker/LLM 或返回客户端。
...
3. Worker 将新 Version 投影改为 `SERVING`、旧 Version 改为 `SUPERSEDED`，记录 Milvus commit timestamp，并用 Strong/guarantee timestamp 查询验证。
...
3. Worker 记录 Milvus commit/guarantee timestamp，以 Strong 查询验证旧授权域不可命中。
...
2. Worker 以至少一次语义消费事件，使用 `inbox_events` 去重；所有本地文件和 Milvus 写入使用稳定业务键、临时文件 + 原子 rename 及 checksum。
3. 新文档版本、权限投影和索引代际在验证成功前都不可发布。
4. 删除采用“先不可见、再异步物理清理”；定时对账本地文件、MySQL、Milvus、缓存和 tombstone。
...
- 引用验证失败：不返回未经验证的生成答案。
- Milvus 不可用：返回系统错误，不改用无 ACL 的备用查询。
...
请求只接受白名单过滤字段：`space_ids`、`corpus_ids`、`category_ids`、`tag_ids`、`product_ids`、`applicable_versions`、`region_codes`、`valid_at`、`language` 和 `mode`。客户端条件只能收窄服务端授权范围。正文、ACL Token、任意 Milvus 表达式和 `tenant_id` 不允许由客户端直接传入。
...
| FR-IDX-03 | MySQL、本地文件、Milvus 定时对账 | 不一致可检测和修复 |
...
- Milvus：采用平台备份/副本满足核心搜索 RTO，同时验证从 MySQL + 规范化产物全量重建；若全量重建超过 4 小时，不得用“可重建”宣称已满足 RTO。
- 配置、Prompt、模型清单和索引元数据进入版本库或配置仓库。
- 发布前完成 MySQL 恢复、对象恢复、Milvus 重建、索引回滚和节点故障演练。
...
- MinerU/OCR 队列堆积；
- Milvus 查询失败或索引代际异常；
- MySQL 与 Milvus 对账不一致；
- 权限投影失败或疑似越权；
- 删除积压、旧任务复活风险；
- Python 本地持久队列、可选 RabbitMQ、Redis、本地文件系统故障、磁盘满、权限错误和备份失败；
- 数据库恢复、版本回滚和紧急下线。
...
- 撤权或删除后 60 秒内停止搜索、问答、预览和下载；24 小时内清除在线 Milvus、Redis、临时区和可访问对象版本。
...
| 契约测试 | AI/MinerU/OCR/ASR/本地文件/MQ/OpenAPI | 每次 PR 用 Stub；夜间真实服务 | 契约或错误语义不兼容 |
| 解析黄金测试 | 各格式正常、复杂、损坏、超限、低质量 | 解析器变更时 | 内容或 locator 回归 |
| 集成测试 | MySQL、Milvus、本地文件、Python 本地持久队列、可选 RabbitMQ、Redis、Worker；Outbox 重复、代际切换、ACL 收紧和删除并发 | 每次合并/每日 | 重复写、路径越界、脏版本、watermark 失效或无法回滚 |
...
| 故障恢复 | 模型、Milvus、MQ、权限服务故障和备份恢复 | 每月、发布前 | 非安全降级或无法恢复 |
...
| G2 独立检索门 | W12 | `/ask` 进入正式验收 | 生产目标 Milvus 原生 BM25、中文 Analyzer、ACL、watermark、索引代际和 `/search` 质量/性能/零越权通过；10 份音频 ASR/时间戳硬门禁通过 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| WBS-00 基线与 Harness | R1 | W1—W2 | 产品负责人 / 技术负责人 | 全部 | 用户技术约束和本机环境已知 | PRD、ADR、威胁模型、60 槽位样本清单/责任、MinerU/Milvus/ACL Harness、容量和测量规程；不宣称真实格式支持 | G0 |
| WBS-10 工程基础 | R1 | W3—W4 | 技术负责人 / 后端+SRE | FR-OPS/AUD/IAM | G0；可用 Stub 身份源 | 仓库、CI/CD、配置/密钥、Python 启动脚本、npm dev、本地文件、MySQL/Milvus/MQ/Redis 原生适配、OIDC、Trace | G1 的平台前置完成 |
...
| WBS-30 索引与检索 | R1 | W9—W12 | 技术负责人 / 搜索+后端 | FR-IDX/RET/IAM | G1；Chunk/ACL Projection v1 冻结 | Embedding、Milvus 原生 BM25、中文 Analyzer、代际追平、watermark、RRF、Reranker、`/search`、小规模试导入 | G2 |
...
| Sprint | 周期 | 演示目标 |
| --- | --- | --- |
| S1 | W1—W2 | ADR、60 槽位采集清单和 MinerU/Milvus/权限 Harness 可复现；不宣称真实格式支持 |
...
| S5 | W9—W10 | Milvus 原生 BM25/Dense、ACL、索引代际；ASR 真实链路 |
...
| 非生产 MySQL/Milvus/可选 MQ/Redis 与本地存储盘 | W9 | 本机原生服务或 Python 本地适配器、路径、权限、容量和监控可用 | Stub 仅开发；真实 Milvus 是 G2 硬门禁 |
...
| Embedding、Milvus、代际 | FR-IDX | WBS-30/50 | 集成、追平、对账、回滚 | G2/G3 |
...
| RAG/搜索/ML 工程师 | 1.75 | 解析、Embedding、Milvus、检索、生成和评测 |
...
| R-02 | Milvus 中文 BM25/ACL/watermark 不达标；Recall 或 p95 未过 G2 | 中/高 | G1 本地词法适配器不得冒充生产；G2 调 Analyzer/授权域，必要时 ADR 引入 OpenSearch 并重基线 | 检索负责人 / G2 | Open |
...
| FastAPI、Python 本地持久队列默认方案、RabbitMQ/Celery 可选原生适配器与模块化单体 ADR | 技术负责人/架构师 | W2 | 脚手架、任务可靠性和排期 |
| ACL 粒度、继承/替换、deny、visibility、Token 上限和超限方案 | 技术+安全+产品 | W1 | 数据模型、Milvus Schema、安全协议 |
...
| 生产目标 Milvus 版本、Collection family、BM25/多语言 Analyzer、ARRAY ACL、RRF 归属和一致性 watermark 的 G2 Harness | 检索负责人 | W2 | G2 是否需要额外搜索组件、索引协议 |
| Embedding、Reranker、LLM、OCR、ASR 候选 | ML 负责人 | W8（G1） | 模型契约、索引维度、成本 |
| 本地存储根目录/备份盘、Python 本地持久队列、可选 RabbitMQ/Redis、OIDC 和原生进程运行方式 | SRE 负责人 | W2 | 环境和 CI/CD |
| `/search`、`/ask`、入库 SLO，控制/内容 RPO，Milvus RTO，在线/备份/审计保留 | 产品+SRE+安全 | W18（G4 前） | 上线承诺和架构成本 |
