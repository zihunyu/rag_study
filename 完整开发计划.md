...
### 当前用户范围覆盖（2026-09-01，优先于下文历史全量基线）

- `APP_SECRET_KEY` 已配置；报告只显示 configured 状态，不输出值。
- ASR/audio 暂缓，`ASR_ENABLED=false`；ASR 三键允许为空，不阻断当前 G4 非 ASR 工作。
- 下文关于音频、ASR、六类 60 份和 6x10 的内容保留为原始完整范围与未来恢复入口，
  不代表当前承诺或当前 Gate 计算。
- 当前 G4 只计算五类非 ASR 难例：文本 PDF、扫描 PDF/图片、DOCX、PPTX、表格，
  各 10 份，共 50 份；audio 标记 `deferred_by_user`。
- 当前剩余阻断仅为五类 50 份合法样本、真实模型预算/授权、MySQL G3/G4 migration
  授权，以及外部 lifecycle 对账/撤权/删除/恢复演练授权。企业 IdP 继续 deferred。
- G5/G6 的代码、SQLite、API、Vue、状态机、Runbook 和合成 Harness 继续完成；这只表示
  `IMPLEMENTATION_COMPLETE_PRE_REAL_VALIDATION`，不得解释为 Pilot/G5/G6 Gate 通过。
- 上述所有真实证据统一在 `FINAL_UNIFIED_VALIDATION` 执行；缺失时最终 acceptance report
  必须 fail-closed 为 BLOCKED，synthetic 数据不能解锁。
- 原始完整 G6 范围包含真实 7 天观察；用户当前将其设为 `deferred_by_user`。观察窗代码、
  API、SQLite、Vue 和 fake-clock 语义测试保留，但不得要求等待 7 天或用它阻断当前验证。
...
| ADR-006 | 所有解析器输出统一文档契约，不向下游暴露 MinerU 私有结构 | G0 已批准方向；G1/G2 契约与合成 Fixture，G4 真实格式门 |
...
G0 不要求真实解析服务或 60 份样本全部到位；退出条件是完成六类采集清单、责任、验收指标和可执行 Harness：文本 PDF、扫描 PDF/图片、DOCX、PPTX、表格、音频各 10 份。G1/G2 只验证契约、解析器、适配器、合成 Fixture 与 Harness，所有结果固定 `real_acceptance=false`；六类真实文件的解析、定位、结构、ASR 时间戳和检索可用性统一在 G4 成为硬门禁，这批难例仍不替代发布黄金集。
...
G0 退出条件是路径候选、降级原则、60 槽位采集清单、责任和 Harness 就绪。G4 真实门禁前不得宣称任何尚未实测格式“已支持”；若出现系统性丢页、错序、表格破坏、时间戳错误或无法定位，必须降级为专用解析器或“受限支持”。
...
- `LLM_ALLOW_HTTP=true` 时 LLM 可使用 HTTP 或 HTTPS，由部署方承担明文传输风险；设为 `false` 时 LLM 与其他外部 AI 服务一样，必须使用 HTTPS 或提供已批准的受信任内网/VPN 传输服务与证据。
- Prompt injection 测试覆盖“忽略指令、泄露其他文档、伪造引用、调用工具”等样例。

### 16.4 平台安全

- 除上述显式 LLM HTTP 例外外，传输使用 TLS；存储静态加密，密钥由 Vault/KMS/平台 Secret 管理和轮换。
...
| G1 入库与契约门 | W8 | 下游用正式 CanonicalDocument/Locator/Chunk v1 验收 | 上传隔离、幂等、状态机、OpenAPI v1；文档/图片/Office/表格解析契约、适配器和合成 Fixture 通过，报告固定 `real_acceptance=false` |
| G2 独立检索门 | W12 | `/ask` 进入正式验收 | Zilliz Cloud 中国区原生 BM25、中文 Analyzer、ACL、watermark、索引代际和 `/search` 质量/性能/零越权通过；音频仅完成契约、适配器、合成 Fixture 与 Harness，`real_acceptance=false` |
| G3 R1 功能冻结 | W16 | 只接收缺陷修复和批准变更 | 问答、引用、反馈、发布/回滚、撤权、删除、治理和管理端完成 |
| G4 Release Candidate | W22 | 申请试点上线 | 当前非 ASR 范围为五类各 10 份真实格式难例；音频/ASR deferred，不计入本轮 ready。其余全量评测、安全、压测、长稳、迁移彩排和恢复演练仍需通过 |
...
| WBS-20 知识与入库 | R1 | W3—W8 | 技术负责人 / 后端+解析 | FR-KM/ING | G0；CanonicalDocument 草案；S2 可用解析 Stub | 领域模型、治理元数据、上传隔离、本地持久任务、PDF/图片/Office/HTML/表格解析、Chunk、契约测试和合成 Fixture；不执行真实样本验收 | G1 |
| WBS-25 音频与结构化适配 | R1 | W7—W12 | 技术负责人 / 解析+ML | FR-ING | G0 路由契约；G4 前真实 ASR 到位 | G1 完成 XLS/XLSX/CSV 结构关系；G2 完成 ASR/时间戳契约、适配器、合成 Fixture 与格式降级；真实难例留到 G4 | G1 表格契约 / G2 音频契约 / G4 真实验收 |
...
| WBS-60A 全格式加固 | R1 | W15—W20 | 产品负责人 / 解析+QA+前端 | FR-ING/UI | G1/G2 契约与适配器冻结；非 ASR 真实难例到位 | OCR、复杂 PDF/Office、表格、`.doc/.ppt` 受限路径、质量评分、最小复核、格式级报告；音频/ASR deferred | G4 当前五类非 ASR 真实格式门槛通过 |
...
| S4 | W7—W8 | G1：表格结构、五类解析契约与合成 Fixture、CanonicalDocument/Locator/Chunk v1 冻结；`real_acceptance=false` |
| S5 | W9—W10 | Zilliz Cloud 原生 BM25/Dense、ACL、索引代际；ASR 契约与适配器 Harness |
| S6 | W11—W12 | G2：Hybrid + RRF + Reranker、security watermark、音频时间戳合成 Fixture 和检索门槛；`real_acceptance=false` |
...
| S11 | W21—W22 | G4 当前范围：五类非 ASR 真实格式门、非 ASR 黄金集、安全、完整迁移彩排、故障和恢复演练；audio deferred |
...
| 当前五类非 ASR 真实格式难例 | W18 | 文本 PDF、扫描/图片、DOCX、PPTX、表格各 10 份可执行，共 50 份 | audio/ASR deferred；原六类 60 份保留为未来完整范围 |
...
| Excel/CSV/音频 | FR-ING/RET（R1） | WBS-20/25/30/60A | G1/G2 契约、适配器和合成 Fixture；G4 真实难例、表格 F1、WER/时间戳、检索可用性 | G1/G2/G4 |
...
| --- | --- | --- | --- | --- | --- |
| R-01 | MinerU/表格/音频格式或定位不达标；G4 真实门禁出现系统性丢页、错序、结构或时间戳错误 | 高/高 | G1/G2 提前完成契约与合成回归；G4 使用 Office 转 PDF、专用表格/ASR 适配器或降为受限支持；仍可能增加硬件和工期 | 解析负责人 / G1、G2、G4 | Open |
...
| R-10 | 表格与音频并入 R1 后吞噬关键路径或缓冲 | 高/高 | 工期重基线为 24 周，设置 G4 六类真实硬门禁和 W21—W22 缓冲；降级必须显式缩小支持范围 | 产品负责人 / G1—G6 | Open |
...
| MinerU/表格/ASR 候选路径、60 槽位清单与 G4 真实验收责任 | 解析负责人 | W2 | 解析架构、硬件和真实 Gate 计划 |
