...
from ragkb.evaluation.fixture_rebuild import fixture_rebuild_plan  # noqa: E402
        ...
        records.append(
            {
                "fixture_id": hashlib.sha256(
                    f"{item['format']}:{sample['id']}".encode()
                ).hexdigest()[:16],
                "text": "",
    ...
    json.dumps(
        {
            ...
            "flagged_count": sum(r["rebuild_required"] for r in plan["records"]),
