...
<template><div class="page libraries-page">
  <div class="page-heading"><div><p class="eyebrow">KNOWLEDGE WORKSPACE</p><h1>你的知识，从这里连接。</h1><p class="page-description">组织文档、沉淀知识，让每个答案都有据可循。</p></div><button class="btn primary" @click="open = true"><Plus :size="18"/>创建知识库</button></div>
  ...
  <div class="section-toolbar"><div class="section-title"><h2>全部知识库</h2><span class="count-pill">{{ filtered.length }}</span></div><div class="toolbar-actions"><label class="search-field"><Search :size="17"/><input v-model="search" aria-label="搜索知识库" placeholder="搜索知识库…"/></label><div class="segmented"><button :class="{ active: layout === 'grid' }" aria-label="卡片视图" :aria-pressed="layout === 'grid'" @click="layout = 'grid'"><LayoutGrid :size="17"/></button><button :class="{ active: layout === 'list' }" aria-label="列表视图" :aria-pressed="layout === 'list'" @click="layout = 'list'"><List :size="18"/></button></div></div></div>
  <div v-if="store.loading && !store.spaces.length" class="library-grid"><div v-for="i in 3" :key="i" class="skeleton-card"/></div>
  <div v-else-if="filtered.length" :class="layout === 'grid' ? 'library-grid' : 'library-list'"><RouterLink v-for="(space, i) in filtered" :key="space.id" :to="`/knowledge-bases/${space.id}/documents`" class="library-card"><div class="library-card-top"><span class="library-symbol" :data-color="i % 3"><BookOpen :size="24"/></span><span class="library-status"><i/>{{ space.processing_count ? '处理中' : '知识库' }}</span><ArrowUpRight :size="18" class="card-arrow"/></div><div class="library-card-body"><h3>{{ space.name }}</h3><p>{{ space.description || '添加文档，为这个知识库建立可靠的知识来源。' }}</p></div><div class="library-card-stats"><span><FileText :size="14"/>{{ space.document_count }} 份文档</span><span><Layers :size="14"/>{{ space.chunk_count }} 个分块</span></div><div class="library-card-footer"><span>{{ space.answerable_count }} 份可问答<span v-if="space.pending_count" class="pending-note"> · {{ space.pending_count }} 份待处理</span></span><small>{{ dateTime(space.updated_ms / 1000) }}</small></div></RouterLink><button v-if="layout === 'grid' && !search" class="create-library-card" @click="open = true"><span><Plus :size="25"/></span><strong>建立新的知识连接</strong><small>创建知识库，开始整理你的资料</small></button></div>
  <EmptyState v-else :title="search ? '没有匹配的知识库' : '建立你的第一个知识库'" :description="search ? '试试其他名称或关键词。' : '上传你的文档，整理成可检索、可追溯的知识。'"><button v-if="!search" class="btn primary" @click="open = true"><Plus :size="17"/>创建知识库</button></EmptyState>
