...
const previewPending = computed(() => versionId.value === doc.value?.version_id && doc.value?.processing_state === 'VALIDATED' && (!quality.value || (quality.value.node_count > 0 && !chunks.value.length)));
const reviewInstructions = computed(() => ['xlsx', 'xls', 'csv'].includes(quality.value?.source_format)
  ? '请抽查工作表是否齐全、表头与行列是否对应、关键数值和单位是否准确。确认这份资料可作为问答来源后，再点击「确认发布」。'
  : '请抽查正文是否完整、标题和表格是否正确、来源位置是否对应原文。确认这份资料可作为问答来源后，再点击「确认发布」。');
...
timer = setInterval(async () => {
  ...
  if ((!['DRAFT','PROCESSING'].includes(doc.value?.processing_state) && !pending) || busy.value || chunkLoading.value || resource.loading.value) return;
  const previous = doc.value.processing_state, selected = versionId.value;
  await load();
  if (selected === versionId.value && (doc.value?.processing_state !== previous || pending)) await readVersion();
}, 4000);
