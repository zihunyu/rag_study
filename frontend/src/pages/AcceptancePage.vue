...
const runName = ref(''), callLimit = ref(300), baseline = ref(''), comparison = ref(null);
...
const counts = computed(() => {
  ...
  const samples = run.value?.repeat_summary?.rows?.flatMap(r => r.samples);
  const verdicts = samples ? samples.map(s => s.verdict) : (run.value?.payload.cases || []).map(c => latest.value[c.id]?.verdict || 'incomplete');
  for (const verdict of verdicts) count[verdict in count ? verdict : 'incomplete']++;
  return count;
});
const failedCaseIds = computed(() => run.value?.repeat_summary?.rows
  ? run.value.repeat_summary.rows.filter(r => r.samples.some(s => s.quality_failed)).map(r => r.case_id)
  : Object.values(latest.value).filter(a => a.verdict === 'failed').map(a => a.case_id));
...
function edit(row = null) {
  ...
  const c = row?.payload || { key: '', category: '业务问答', question: '', reading: { mode: 'fact', document_ids: [] },
    history: [], expected_status: 'answered', required_points: [], forbidden_claims: [], check_relevance: true, check_citation_structure: true,
...
async function createRun(ids = selected.value) {
  ...
  const body = { name: runName.value.trim() || `验收 ${new Date().toLocaleString('zh-CN')}`, case_ids: ids, call_limit: Number(callLimit.value), review_mode: reviewMode.value, repeat_count: Number(repeatCount.value) };
