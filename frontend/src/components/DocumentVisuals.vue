...
const inheritedCount = computed(() => assets.value.filter(a => a.inherited_from_version_id).length);
...
<nav v-if="assets.length" class="visual-gallery" aria-label="文档图片导航"><button v-for="(asset,i) in assets" :key="asset.id" :class="{ active: active === asset.id }" @click="active = asset.id">图片 {{ i + 1 }}<span :class="['stage-dot', asset.status === 'verified' ? 'success' : 'running']"/><small>{{ asset.inherited_from_version_id ? '沿用 · ' : '' }}{{ labels[asset.stage] || (asset.status === 'verified' ? '处理完成' : '待复核') }}</small></button></nav>
